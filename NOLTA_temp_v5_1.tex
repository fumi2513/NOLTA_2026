...
In this study, we analyze the generalization process of ML by comparing ML models with individual learning (IL) models. We focus on the double descent phenomenon because it contradicts the conventional bias-variance trade-off and involves non-monotonic changes in test error, which can reveal characteristics of the generalization process. We first examine differences in double descent behavior between ML and IL with respect to the number of model parameters and training epochs. As further analyses, we compare ML and IL in terms of variance, loss landscape geometry, and Hessian-based properties, especially across the regimes of double descent.
